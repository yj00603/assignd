...
void setup() {
  ...
  soilEmptyX = new float [48];
  soilEmptyY = new float [48];
  for(int i = 0; i < 48; i ++){
    int count =  1  +  floor(random(2));
     for(int j = 0; j < count; j ++){
       soilEmptyX[i] = floor(random(7));
       soilEmptyY[i] = i+1;
      }
   }
  ...
  for(int i = 0; i < 6; i ++){
    soldierX[i] = floor(random(7));
     for(int j = 0; j < 24; j +=4){
       soldierY[i] = (i+floor(random(3)));
      }
   }
  ...
  for(int i = 0; i < 6; i ++){
       cabbageX[i] = floor(random(7));
     for(int j = 0; j < 24; j +=4){
       cabbageY[i] = (i+floor(random(3)));
      }
   }
}
...
void draw() {
  ...
  switch (gameState) {
    ...
    for(int a = 0; a < 8; a ++){
      int y1 = a*SOIL_SIZE;
        image(stones[0][4],a*SOIL_SIZE,y1); 
    }
    
    //second stone
    
    for(int x2 = 0; x2 < SOIL_SIZE*2; x2 +=SOIL_SIZE){
       for(int b = 8; b < 16; b++){
         int y2 = b*SOIL_SIZE;
         if(b==8 ||b==11 ||b==12 ||b==15){
           image(stones[0][4], x2+SOIL_SIZE,y2);
           image(stones[0][4],x2+SOIL_SIZE*5,y2);
         }
         if(b==9 ||b==10 ||b==13 ||b==14){
           image(stones[0][4],x2-SOIL_SIZE,y2);
           image(stones[0][4],x2+SOIL_SIZE*3,y2);
           image(stones[0][4],x2+SOIL_SIZE*7,y2);
         }
       }
    }
    ...
    int y3;
    for(int x3 = 0; x3 <SOIL_SIZE*8; x3 += SOIL_SIZE*3){
       for(int c = 16; c < 24; c++){
          y3 = c*SOIL_SIZE ;
          if(c==16 ||c==19 ||c==22){
            image(stones[0][4],x3+SOIL_SIZE,y3);
            image(stones[0][4],x3+SOIL_SIZE*2,y3);
            image(stones[1][4],x3+SOIL_SIZE*2,y3);
          }
          if(c==17 ||c==20 ||c==23){
            image(stones[0][4],x3,y3);
            image(stones[0][4],x3+SOIL_SIZE,y3);
            image(stones[1][4],x3+SOIL_SIZE,y3);
          }
          if(c==18 ||c==21){
            image(stones[0][4],x3,y3);
            image(stones[0][4],x3+SOIL_SIZE*2,y3);
            image(stones[1][4],x3,y3);
          }
       }
    }
  }
    ...
    for(int i = 0; i < 48; i++){
      image(soilEmpty,soilEmptyX[i]*SOIL_SIZE,soilEmptyY[i]*SOIL_SIZE);
      if(playerX == soilEmptyX[i]*SOIL_SIZE && playerY+SOIL_SIZE == soilEmptyY[i]*SOIL_SIZE ){
